  ...
  const load = useCallback(async () => {
    ...
    try {
      const list = await fetchAllOffsetPages(async (offset, limit) => {
        const res = await getExamPrepUnitPractices(unitId, { limit, offset })
    ...
    <div className="space-y-6">
      ...
      {filteredPractices.length === 0 ? (
        ...
        <PracticeSelectableGrid
          ...
          onReload={load}
          unlinkActionLabel="Remove from unit"
      ...
      >
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle>Remove from this unit?</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-grayScale-600">
            ...
            <span className="font-semibold text-grayScale-800">{unitName}</span>.
            {practiceToUnlink &&
            parentsFromPractice(practiceToUnlink).filter(
              (p) =>
                !(
                  p.parent_kind === unitParent.parent_kind &&
                  p.parent_id === unitParent.parent_id
                ),
            ).length === 0
