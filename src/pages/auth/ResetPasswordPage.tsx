...
import {
  parseVerifyPasswordReset,
  resetTeamPassword,
  verifyTeamPasswordReset,
} from "../../api/team.api"
...
import {
  getInvalidResetLinkDescription,
  getInvalidResetLinkTitle,
  mapTeamPasswordResetError,
} from "../../lib/teamPasswordReset"
...
export function ResetPasswordPage() {
  ...
  const [verifyState, setVerifyState] = useState<"loading" | "invalid" | "ready">(
    hasLinkParams ? "loading" : "ready",
  )
  ...
  const loadVerification = useCallback(async () => {
    ...
    try {
      const res = await verifyTeamPasswordReset(emailFromQuery, otpFromQuery)
      const data = parseVerifyPasswordReset(res)

      if (!data || data.valid !== true) {
        setInvalidTitle(getInvalidResetLinkTitle(data))
        setInvalidDescription(getInvalidResetLinkDescription(data))
      ...
      setInvalidTitle("This reset link is invalid")
      setInvalidDescription(
        "The link may be expired, invalid, or already used. Request a new one from Forgot password.",
      )
      setVerifyState("invalid")
    }
  }, [emailFromQuery, hasLinkParams, otpFromQuery])
        ...
        <div className="w-full max-w-[420px]">
          ...
          <div className="mb-8">
            ...
            <h1 className="mb-2 text-3xl font-bold tracking-tight text-grayScale-600">
              {verifyState === "invalid" ? invalidTitle : "Reset password"}
            </h1>
          ...
          {verifyState === "loading" ? (
            <div className="flex flex-col items-center gap-3 py-16">
              ...
              <p className="text-sm text-grayScale-400">Verifying reset link…</p>
            </div>
          ) : null}
          ...
          {verifyState === "invalid" ? (
            <div className="space-y-4 rounded-xl border border-red-200 bg-red-50 px-4 py-4">
              ...
              <Link
                to="/forgot-password"
                className="inline-block text-sm font-semibold text-red-800 underline"
              >
